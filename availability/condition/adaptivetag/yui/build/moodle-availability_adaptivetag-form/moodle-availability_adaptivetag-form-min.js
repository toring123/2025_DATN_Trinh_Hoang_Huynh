YUI.add('moodle-availability_adaptivetag-form', function(Y) {
...
M.availability_adaptivetag.form.getNode = function(json) {
    var html = '<label><span class="pr-3">' + M.util.get_string('tag', 'availability_adaptivetag') + '</span> <select class="custom-select" name="tag" title="' + M.util.get_string('tag', 'availability_adaptivetag') + '">';
    html += '<option value="">Choose...</option>';
    if (this.availableTags) {
        for (var i = 0; i < this.availableTags.length; i++) {
            var tag = this.availableTags[i];
            var selected = (json.tag === tag) ? ' selected' : '';
            html += '<option value="' + Y.Escape.html(tag) + '"' + selected + '>' + Y.Escape.html(tag) + '</option>';
        }
    }
    html += '</select></label>';
    html += '<br><label class="form-inline"><span class="pr-3">' + M.util.get_string('mincompletions', 'availability_adaptivetag') + '</span> <input type="number" class="form-control" name="mincompletions" min="1" value="' + (json.mincompletions || 1) + '" style="width: 80px;" title="' + M.util.get_string('mincompletions', 'availability_adaptivetag') + '"/></label>';
